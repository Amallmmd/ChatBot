...
entryForm.addEventListener('submit', async (e) => {
    ...
    // Check contradiction
    const checkRes = await fetch(`${API_BASE}/check_contradiction`, {
        method: 'POST',
    ...
    if (checkRes.is_contradiction) {
        ...
        // Add entry directly
        await fetch(`${API_BASE}/add_entry`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
showDataBtn.addEventListener('click', async () => {
    if (noonDataDiv.style.display === 'none') {
        const data = await fetch(`${API_BASE}/get_noon_data`).then(r => r.json());
...
sendChatBtn.addEventListener('click', async () => {
    ...
    const res = await fetch(`${API_BASE}/chat_response`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
function renderNoonData(data) {
    ...
    let html = '<table border="1"><tr><th>Vessel</th><th>Date</th><th>Laden/Ballast</th><th>Report Type</th></tr>';
    data.forEach(row => {
